...
def get_fetched_log_keys(sgg_code: str, deal_ymd: str) -> set:
    ...
    Returns:
        set: {'rent_apt', 'rent_rh', 'rent_offi'} 중 수집 완료된 항목
    """
    engine = get_engine()
    try:
            ...
            result = conn.execute(
                text("""
                    ...
                    WHERE sigungu_code = :sgg_code
                      ...
                      AND data_type    IN ('rent_apt', 'rent_rh', 'rent_offi')
                """),
...
def mark_as_fetched(sgg_code: str, deal_ymd: str, data_type: str) -> None:
    ...
    Args:
        ...
        data_type: 'rent_apt' | 'rent_rh' | 'rent_offi'
    """
    engine = get_engine()
    try:
        with engine.connect() as conn:
...
def is_fully_fetched(sgg_code: str, deal_ymd: str) -> bool:
    """
    해당 (지역, 월)의 전월세 3개 유형이 모두 수집됐는지 확인

    Returns:
        bool: 3개 유형 모두 완료 시 True
    """
    fetched = get_fetched_log_keys(sgg_code, deal_ymd)
    return fetched >= set(BUILDING_TYPE_TO_LOG_KEY.values())
...
def parse_rent_xml_to_df(xml_text: str, code_map: dict, building_type: str) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: 파싱된 전월세 데이터 (0건이면 빈 DataFrame)
    """
    try:
    ...
    for item in items:
        ...
        records.append({
            ...
            'floor':             item.findtext('floor', '1').strip(),
...
def fetch_rent_data_and_save(lawd_cd: str, deal_ymd: str, code_map: dict) -> bool:
    ...
    - DB 저장은 INSERT IGNORE 방식으로 중복 방지
    ...
    for building_type, api_url in API_URLS_RENT.items():
        ...
        try:
            ...
            df = parse_rent_xml_to_df(response.text, code_map, building_type)

            if df.empty:
                print(f"  -> {building_type}: 0건.")
            else:
                # INSERT IGNORE: DB 유니크 인덱스 기준 중복 행은 자동 무시
                with engine.connect() as conn:
                    df.to_sql(
                        RENT_TABLE_NAME,
                        con=conn,
...
def get_all_pending(regions: list) -> dict:
    ...
    api_price_log에서 3개 유형(rent_apt, rent_rh, rent_offi)이
    ...
    """
    engine = get_engine()
    all_log_keys = set(BUILDING_TYPE_TO_LOG_KEY.values())  # {'rent_apt', 'rent_rh', 'rent_offi'}
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                ...
                FROM api_price_log
                WHERE data_type IN ('rent_apt', 'rent_rh', 'rent_offi')
...
def main_fetch_loop():
    """
    api_price_log 기반 라운드 로빈 전월세 데이터 수집
    ...
    print(f"--- [전월세] api_price_log 기반 데이터 수집 시작 ---")
    ...
    try:
        while call_count < API_CALL_LIMIT_PER_RUN:
            print(f"\n--- [전월세] 새 라운드 시작 (현재 호출: {call_count}/{API_CALL_LIMIT_PER_RUN}) ---")
            ...
            for sgg_code in regions:
                ...
                success = fetch_rent_data_and_save(sgg_code, date_ym, code_map)
                if not success:
    ...
    print(f"\n--- [전월세] 수집 세션 종료 (총 {call_count}회 호출) ---")
