...
def _create_join_key_from_unique_no(unique_no):
    ...
    try:
        ...
        # 하이픈(-)으로 분리
        # parts[0]: '2823710100' (district 5자리 + legal_dong 5자리)
        # parts[1]: '3'
        # parts[2]: '00650124' (main_jibun 4자리 + sub_jibun 4자리)
...
def _create_join_key_from_address(address_str):
    ...
    try:
        ...
        # 1. 마지막 공백을 기준으로 나눔 (앞부분: district+동, 뒷부분: 번지)
        # 예: "인천광역시 부평구 부평동", "65-124"
        parts = address_str.rsplit(' ', 1)
        if len(parts) != 2:
            return None

        addr_part = parts[0] # district + 동
        bunji_part = parts[1] # 65-124

        # 2. district와 동 분리 (마지막 공백 기준)
        # 예: "인천광역시 부평구", "부평동"
