...
public class PopulationDataLoader {
    ...
    public boolean loadData() {
        try {
            ...
            if (!isChanged) {
                log.info("[Population] 파일 변경 없음. 적재를 건너뜁니다.");
                return false;
            }

            log.info("[Population] 파일 변경 감지. 데이터 동기화를 시작합니다...");

            // 인구 업데이트 SQL (행정동 코드가 일치하는 행의 인구수와 기준년월 업데이트)
            String sql = "UPDATE safety_regions SET population = ?, population_stats_ym = ? WHERE sgis_code = ?";
            ...
            log.info("[Population] 데이터 동기화 완료. (총 {}건 추가/업데이트)", processedCount);
            return true;
